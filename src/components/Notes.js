...
const Notes = (props) => {
  ...
  const [initialGetNotes, setInitialGetNotes] = useState(true);
  ...
  useEffect(() => {
    if (localStorage.getItem("token") && initialGetNotes) {
      getNotes();
      setInitialGetNotes(false);
    } else if (!localStorage.getItem("token")) {
      navigate("/login");
    }
  }, [initialGetNotes, getNotes, navigate]);
      ...
      <div className="row pt-3 px-0 mx-0">
        ...
        <div className="col-12 col-md-8 col-lg-8 mx-0 px-0">
          <div className="row mx-0 px-0 py-3">
            <h3 className="pb-2">Your notes</h3>
            ...
            <div className="modal-body">
              <form>
                <div className="mb-3">
                  ...
                  <input type="text" className={`form-control ${theme === "dark" ? "form-control-dark" : ""}`} id="etitle" name="etitle" aria-describedby="emailHelp" onChange={onChange} value={note.etitle} minLength={5} required />
                </div>
                <div className="mb-3">
                  <label htmlFor="edescription" className="form-label">
                    Description
                  </label>
                  <input type="text" className={`form-control ${theme === "dark" ? "form-control-dark" : ""}`} id="edescription" name="edescription" onChange={onChange} value={note.edescription} minLength={5} required />
                </div>
                <div className="mb-3">
                  <label htmlFor="etag" className="form-label">
                    Tag
                  </label>
                  <input type="text" className={`form-control ${theme === "dark" ? "form-control-dark" : ""}`} id="etag" name="etag" onChange={onChange} value={note.etag} />
                </div>
              </form>
            </div>
